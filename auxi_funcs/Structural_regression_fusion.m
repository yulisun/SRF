...
inv_TfxmuI = inv(4*Lf*betax + u2 * eye(N));% (4*beta*Lf+u2*I)^-1
inv_TfymuI = inv(4*Lf*betay + u2 * eye(N));% (4*beta*Lf+u2*I)^-1
for i=1:Niter
    deltx_old = deltx;
    delty_old = delty;
